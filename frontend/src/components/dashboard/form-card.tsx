...
export function FormCard({ form, index, viewMode = "list" }: FormCardProps) {
  ...
  const actionsMenu = (
    <DropdownMenu>
      <DropdownMenuTrigger className="h-8 w-8 flex items-center justify-center rounded-md hover:bg-slate-100 transition-all cursor-pointer text-slate-500">
      ...
      <DropdownMenuContent align="end" className="w-48">
        ...
        <DropdownMenuSeparator />
        <DropdownMenuItem onClick={() => setShowDelete(true)} className="text-destructive focus:text-destructive cursor-pointer">
    ...
    <>
      {viewMode === "list" ? (
        ...
        >
          ...
          {/* Response count */}
          <div className="w-24 text-center text-sm text-slate-500">-</div>
          ...
          {/* Updated date */}
          <div className="w-32 text-right text-sm text-slate-500">{updatedDate}</div>
        ...
        <motion.div
          ...
          transition={{ duration: 0.2, delay: index * 0.04 }}
          className="group relative flex flex-col rounded-xl border border-border/60 bg-white hover:border-primary/30 hover:shadow-md transition-all overflow-hidden"
        >
          {/* Card preview area */}
          <Link href={`/builder/${form.id}`} className="block">
            <div className="h-36 bg-gradient-to-br from-slate-50 to-rose-50/40 flex items-center justify-center border-b border-border/40">
            ...
            {/* Card info */}
            <div className="p-3">
              <h3 className="font-medium text-sm text-slate-900 truncate group-hover:text-primary transition-colors">
                {form.title}
              </h3>
              <p className="text-xs text-slate-400 mt-1">{updatedDate}</p>
            </div>
          </Link>
